...
export const listAllBusinesses = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabaseAdmin.from("businesses").select("id, name, slug, category, phone, owner_id, created_at, status, monthly_fee_cents").order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
