...
async function requireMaster(accessToken: string) {
  ...
  if (userError || !userData.user) {
    throw new Error("Sessão Master inválida. Entre novamente no acesso Master.");
  }
...
export const getMasterStatus = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => z.object({ accessToken: accessTokenInput }).parse(data))
  .handler(async ({ data }) => {
    await requireMaster(data.accessToken);
    return { isMaster: true, hasMaster: true };
  });
...
export const listAllBusinesses = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: appointments } = await supabaseAdmin.from("appointments").select("business_id");
    const counts = new Map<string, number>();
    for (const appointment of appointments ?? []) {
      counts.set(appointment.business_id, (counts.get(appointment.business_id) ?? 0) + 1);
    }
    ...
    return (businesses ?? []).map((business) => {
      const owner = (owners ?? []).find((item) => item.id === business.owner_id);
      const monthPayment = (payments ?? []).find(
        (payment) =>
          payment.business_id === business.id &&
          payment.reference_month.slice(0, 7) === currentMonth,
      );
      return {
        ...business,
        owner_name: owner?.full_name ?? null,
        owner_login: owner?.email ?? null,
        appointments: counts.get(business.id) ?? 0,
...
export const createBusinessWithOwner = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (created.error) {
      ...
      ownerId = existing.id;
      const updated = await supabaseAdmin.auth.admin.updateUserById(ownerId, {
        password: phonePassword(data.password),
      });
...
export const deleteBusiness = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) =>
    z.object({ accessToken: accessTokenInput, id: z.string().uuid() }).parse(data),
  )
  .handler(async ({ data }) => {
    const supabaseAdmin = await requireMaster(data.accessToken);
...
export const getPlatformMetrics = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const active = list.filter((business) => business.status !== "suspenso");
    const { data: payments } = await supabaseAdmin
      .from("subscription_payments")
    ...
    const paidThisMonth = (payments ?? []).filter(
      (payment) =>
        payment.status === "pago" && payment.reference_month.slice(0, 7) === currentMonth,
    );
    ...
    const depositsTotal = (deposits ?? [])
      .filter((deposit) => deposit.status === "pago" || deposit.status === "aprovado")
      .reduce((sum, deposit) => sum + (deposit.amount_cents ?? 0), 0);
    ...
    return {
      ...
      mrrCents: active.reduce((sum, business) => sum + (business.monthly_fee_cents ?? 0), 0),
      paidThisMonthCount: paidThisMonth.length,
      paidThisMonthCents: paidThisMonth.reduce(
