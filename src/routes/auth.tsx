  ...
  return (
    <main className="relative flex min-h-[100svh] items-center justify-center overflow-hidden bg-[#f8fafc] px-4 py-8 text-[#111827] sm:px-6">
      <section className="relative z-10 w-full max-w-[430px]">
        ...
        <div className="rounded-2xl border border-[#e5e7eb] bg-transparent p-6 shadow-none sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="group relative"><LockKeyhole className="pointer-events-none absolute left-3.5 top-1/2 size-[18px] -translate-y-1/2 text-[#9ca3af] group-focus-within:text-[#1677ff]" /><input id="password" type={showPassword ? "text" : "password"} inputMode="numeric" autoComplete="current-password" value={password} onChange={(event) => setPassword(onlyDigits(event.target.value).slice(0, 4))} placeholder="••••" minLength={4} maxLength={4} required className="h-12 w-full rounded-lg border border-[#d1d5db] bg-transparent pl-11 pr-12 text-[16px] tracking-[0.22em] text-[#111827] outline-none transition focus:border-[#1677ff] focus:ring-4 focus:ring-blue-500/10 placeholder:text-[#9ca3af]" /><button type="button" onClick={() => setShowPassword((visible) => !visible)} aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#9ca3af] hover:text-[#4b5563]">{showPassword ? <EyeOff className="size-[18px]" /> : <Eye className="size-[18px]" />}</button></div>
            </div>
