...
type Form = { id?: string; name: string; duration: string; price: string; deposit: string; description: string; isCombo: boolean; showPrice: boolean; showDuration: boolean; showService: boolean; imagePath: string | null; professionalIds: string[] };
const empty: Form = { name: "", duration: "30", price: "0", deposit: "0", description: "", isCombo: false, showPrice: true, showDuration: true, showService: true, imagePath: null, professionalIds: [] };
...
function ServicosPage() {
  ...
  const { data: people } = useQuery({ queryKey: ["professionals", businessId], enabled: !!businessId, queryFn: async () => { const { data, error } = await supabase.from("professionals").select("id,name,active").eq("business_id", businessId!).eq("active", true).order("name"); if (error) throw error; return data; } });
  ...
  const save = useMutation({ mutationFn: async () => {
    ...
    const result = form.id ? await supabase.from("services").update(payload).eq("id", form.id).select("id").single() : await supabase.from("services").insert(payload).select("id").single();
  ...
  const edit = (s: NonNullable<typeof services>[number]) => { setForm({ id: s.id, name: s.name, duration: String(s.duration_minutes), price: money(s.price_cents), deposit: money(s.deposit_cents), description: s.description ?? "", isCombo: s.is_combo, showPrice: s.show_price, showDuration: s.show_duration, showService: s.show_service, imagePath: s.image_path, professionalIds: (links ?? []).filter((l) => l.service_id === s.id).map((l) => l.professional_id) }); setOpen(true); };
  const upload = async (file?: File) => { if (!file || !businessId) return; setUploading(true); const path = `${businessId}/services/${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9.]/g, "-")}`; const { error } = await supabase.storage.from(LOGO_BUCKET).upload(path, file, { upsert: false }); setUploading(false); if (error) return toast.error(error.message); setForm((v) => ({ ...v, imagePath: path })); };
  ...
  return <div><PageHeader title="Serviços" subtitle="Configure como cada serviço aparece para o cliente." action={<Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setForm(empty); }}><DialogTrigger asChild><Button><Plus className="size-4" /> Novo serviço</Button></DialogTrigger><DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>{form.id ? "Editar serviço" : "Cadastrar serviço"}</DialogTitle></DialogHeader>
    <Tabs defaultValue="dados"><TabsList className="grid w-full grid-cols-3"><TabsTrigger value="dados">Dados</TabsTrigger><TabsTrigger value="vinculos">Vínculos</TabsTrigger><TabsTrigger value="imagem">Imagem</TabsTrigger></TabsList>
      ...
      <TabsContent value="vinculos" className="space-y-3 pt-4"><p className="text-sm text-muted-foreground">Escolha quem pode realizar este serviço.</p>{people?.map((p) => <label key={p.id} className="flex items-center gap-3 rounded-md border border-border p-3"><Checkbox checked={form.professionalIds.includes(p.id)} onCheckedChange={(v) => setForm({ ...form, professionalIds: v ? [...form.professionalIds, p.id] : form.professionalIds.filter((id) => id !== p.id) })} /><span>{p.name}</span></label>)}{!people?.length && <p className="py-8 text-center text-sm text-muted-foreground">Cadastre profissionais para criar vínculos.</p>}</TabsContent>
    ...
    </Tabs><DialogFooter><Button onClick={() => save.mutate()} disabled={!form.name.trim() || save.isPending || uploading}>{uploading ? "Enviando..." : "Salvar serviço"}</Button></DialogFooter></DialogContent></Dialog>}</PageHeader>
