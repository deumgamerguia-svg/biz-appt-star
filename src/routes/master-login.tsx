...
const SESSION_COOKIE = "agenda_supabase_session";

function syncServerSession(accessToken: string) {
  if (typeof document === "undefined" || !accessToken) return;
  const secure = window.location.protocol === "https:" ? "; Secure" : "";
  document.cookie = `${SESSION_COOKIE}=${encodeURIComponent(accessToken)}; Path=/; SameSite=Lax${secure}`;
}
...
export const Route = createFileRoute("/master-login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Acesso exclusivo ao responsável pela plataforma Agenda Agora.",
      },
    ],
  }),
...
function MasterLoginPage() {
  ...
  }, [loading, user, session?.access_token, navigate]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!authData.user || !authData.session?.access_token) {
        throw new Error("Não foi possível iniciar a sessão Master.");
      }
      ...
      syncServerSession(authData.session.access_token);
      ...
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Área exclusiva do responsável pela plataforma.
          </p>
        </div>
        <div className="surface p-7">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={busy}>
              {busy ? "Validando..." : "Entrar no Master"}
            </Button>
          </form>
          <Link
            to="/auth"
